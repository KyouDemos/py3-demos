...
print('s[::2]: ', s[::2])  # 序列分片（步长为二，即从左向右隔一个取一个）
print('s[::-1]:', s[::-1])  # 序列分片（步长为负一，表示从右向左，即字符串翻转）
print('s[4:1:-1]: ', s[::-1])  # 序列分片（步长为负数时，步进方向调转，注意开始和截止的坐标）
...
print("chr(65):", chr(65))  # 字母的ASCII编码
